...
export default function MerchantDashboard({ onNavigate }) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn select-none">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white border border-slate-200/90 p-6 rounded-2xl shadow-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5">
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Revenue Recovery Dashboard</h2>
            <span className="px-2.5 py-0.5 rounded-full bg-indigo-50 text-indigo-700 text-xs font-semibold border border-indigo-100 font-mono">
              Merchant ID: {dashboardData?.merchant_id || CURRENT_MERCHANT_ID}
            </span>
          </div>
          <p className="text-xs text-slate-500">
            <strong className="text-rose-600 font-bold">{formattedRiskText} at risk</strong> across {dashboardData?.failed_payments || 0} failed payments. RecoverAI has {dashboardData?.active_recovery_cases || 0} active recovery workflows.
          </p>
        </div>

        <button
          onClick={() => onNavigate('copilot')}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-all shadow-sm shadow-indigo-600/20"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white border border-slate-200/90 p-5 rounded-2xl space-y-3 shadow-sm hover:border-slate-300 transition-colors">
        ...
        <div className="bg-white border border-slate-200/90 p-5 rounded-2xl space-y-3 shadow-sm hover:border-slate-300 transition-colors">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Active Workflows</span>
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 text-xs text-cyan-600 font-semibold mt-1">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-3">
                <h3 className="text-base font-bold text-slate-900">Recent Failed Transactions</h3>
                <button
              ...
              <p className="text-xs text-slate-500">Click any transaction to inspect AI diagnostic breakdown</p>
            </div>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:flex-none">
                ...
                <input
                  ...
                  className="w-full sm:w-48 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 pl-8 pr-3 py-1.5 focus:outline-none focus:border-indigo-500"
                />
              </div>

              <button
                onClick={() => onNavigate('denials')}
                className="px-3 py-1.5 rounded-xl bg-slate-100 border border-slate-200 text-xs font-semibold text-slate-700 hover:text-slate-900 hover:bg-slate-200 transition-colors"
              >
                View Denials
              </button>
            </div>
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {filteredPayments.slice(0, 8).map((payment) => {
                    ...
                    return (
                      ...
                      >
                        <td className="py-3.5 px-4 font-mono font-semibold text-slate-900 group-hover:text-indigo-600">
                          {payment.id}
                        </td>
                        <td className="py-3.5 px-4 font-medium text-slate-800">{payment.customer}</td>
                        <td className="py-3.5 px-4 font-bold text-slate-900">
                          ₹{payment.amount.toLocaleString('en-IN')}
                        </td>
                        <td className="py-3.5 px-4 text-slate-600">{payment.method}</td>
            ...
            {merchantActivityFeed.map((item) => (
              <div key={item.id} className="flex items-start gap-3 pb-3 border-b border-slate-100 last:border-0 last:pb-0">
                ...
                <div className="space-y-0.5 flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 leading-snug truncate">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
